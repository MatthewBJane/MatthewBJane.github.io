...
Using the `brm` function in the `brms` [@brms] package we can construct the meta-regression GAM model by utilizing the `se()` function to incorporate the sampling errors in observed effects and the `s()` function to tell brms you want to model a GAM.
...
We can plot out the models by displaying variability in the estimate of the conditional expectation with posterior draws.
