...
In order to calculate the pre/post correlation ($r$), we need an estimate of the standard deviation (SD) of pre-test scores ($S_{pre}$), the SD of post-test scores ($S_{post}$), and the SD of change scores ($S_{change}$, where $x_{change}=x_{post}-x_{pre}$). If the post-test SD is unavailable, but the pre-test SD is available, you can approximate the post-test SD by taking average ratio of the pre-test SD and post-test SD from all other studies in the current meta-analysis.
